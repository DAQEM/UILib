...
public class RepeatingTextureComponent extends AbstractComponent<RepeatingTextureComponent> {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta, int color) {
        graphics.blit(RenderType::guiTextured, getTexture().getTextureLocation(), 0, 0, getTexture().getX(), getTexture().getY(), getWidth(), getHeight(), getTexture().getWidth(), getTexture().getHeight());
    }
}
